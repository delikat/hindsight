...
| `HINDSIGHT_API_LLM_REASONING_EFFORT` | Reasoning effort for providers/models that support it (for example `none`, `low`, `medium`, `high`, `xhigh`). Set it and the value is sent as given, whatever your model is called — which is how you control thinking-token volume on a self-hosted reasoning model (vLLM, Ollama, llama.cpp, TGI), where `none` is often the only value that removes the thinking block. Leave it unset and no reasoning parameter is sent at all, so each model runs at its own default effort. Honoured by `openai` and every OpenAI-compatible provider, `openai-responses`, `openai-codex`, `xai`, `llamacpp`, `litellm`/`litellmrouter` (which translate it per target provider), and the native `gemini`/`vertexai` providers on Gemini 3 and newer, where it becomes the request's `thinking_level` (`none`/`minimal` → `MINIMAL`, `low`, `medium`, `high`, `xhigh` → `HIGH`; unset sends no level, so the model runs at its own default — `MEDIUM` on the 3.x Flash line). Gemini 3.7 Flash rejects `MINIMAL`, so `none` is not available there. Gemini 1.x/2.x have no level parameter and, like the `anthropic` and `claude-code` providers, log a warning at startup if you set one — reach those through `litellm` to apply it. | Unset (model's own default) |
